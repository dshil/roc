...
bool UDPReceiverPort::open() {
    ...
    if (address_.multicast() && address_.has_multicast_iface()) {
...
bool UDPReceiverPort::join_multicast_group_() {
    if (int err = uv_udp_set_membership(&handle_, packet::ip_to_str(address_).c_str(),
                                        packet::multicast_iface_to_str(address_).c_str(),
                                        UV_JOIN_GROUP)) {
    ...
    roc_log(LogDebug, "udp receiver: joined multicast group %s at interface %s",
            packet::ip_to_str(address_).c_str(),
            packet::multicast_iface_to_str(address_).c_str());
...
void UDPReceiverPort::leave_multicast_group_() {
    ...
    roc_log(LogDebug, "udp receiver: left multicast group %s at interface %s",
            packet::ip_to_str(address_).c_str(),
            packet::multicast_iface_to_str(address_).c_str());
}
